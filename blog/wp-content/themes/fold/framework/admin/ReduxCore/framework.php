...
if( !class_exists( 'ReduxFramework' ) ) {
    ...
    class ReduxFramework {
        ...
        public $framework_url       = 'http://www.rtl-theme.com/';
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( empty( $this->args['footer_credit'] ) ) {
                $this->args['footer_credit'] = '<span id="footer-thankyou">پنل تنظیمات فارسی شده توسط <a href="'.esc_url( $this->framework_url ).'" target="_blank">راست چین</a></span>';
            }
            if ( empty( $this->args['menu_title'] ) ) {
                $this->args['menu_title'] = __( 'Options', 'zoomarts' );
            }
            if ( empty( $this->args['page_title'] ) ) {
                $this->args['page_title'] = __( 'Options', 'zoomarts' );
            }
        ...
        public function _internationalization() {
             ...
             * @param string     'zoomarts'  text domain
             */
            $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'zoomarts' );
            ...
            load_textdomain( 'zoomarts', dirname( __FILE__ ) . '/languages/' . $locale . '.mo' );
...
//                $comparison = $field['required'][1];
        ...
        function _admin_bar_menu()
        {
            ...
            if($menu){
                ...
                $nodeargs = array(
                    ...
                    'title' => "<span class='ab-icon dashicons-admin-generic'></span> " . __('Theme Options', 'zoomarts') ." " . $theme_data->get('Name') ,
        ...
        public function _enqueue() {
            ...
            $this->localize_data['args'] = array(
                'save_pending'          => __( 'You have changes that are not saved. Would you like to save them now?', 'zoomarts' ),
                'reset_confirm'         => __( 'Are you sure? Resetting will lose all custom values.', 'zoomarts' ),
                'reset_section_confirm' => __( 'Are you sure? Resetting will lose all custom values in this section.', 'zoomarts' ),
                'preset_confirm'        => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'zoomarts' ),
                'please_wait'        => __( 'Please Wait', 'zoomarts' ),
        ...
        public function get_default_output_string($field) {
            ...
            if ( !empty( $default_output ) ) {
                $default_output = __( 'Default', 'zoomarts' ) . ": " . substr($default_output, 0, -2);
            }
            if (!empty($default_output)) {
...
//                                $field['validate'] = "color_rgba";
  //                          }
        ...
        public function _options_page_html() {
            ...
            // Do we support JS?
            echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'zoomarts' ) . '</div></noscript>';
            ...
            echo '<a href="javascript:void(0);" class="expand_options'.$expanded.'">' . __( 'Expand', 'zoomarts' ) . '</a>';
            echo '<div class="redux-action_bar">';
            submit_button( __( 'Save Changes', 'zoomarts'), 'primary', 'redux_save', false );
            echo '&nbsp;';
            submit_button( __( 'Reset Section', 'zoomarts'), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
            echo '&nbsp;';
            submit_button( __( 'Reset All', 'zoomarts' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';

            echo '<div class="redux-ajax-loading" alt="' . __( 'Working...', 'zoomarts' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';
            ...
            if( isset( $this->transients['last_save_mode'] ) ) {
                ...
                if( $this->transients['last_save_mode'] == "import" ) {
                     ...
                     */
                    echo '<div id="saved_notice" class="admin-notice notice-blue"><strong>' . apply_filters( "redux-imported-text-{$this->args['opt_name']}", __( 'Settings Imported!', 'zoomarts' ) ) . '</strong></div>';
                    //exit();
                     ...
                     * @param string  translated "settings imported" text
                     */
                    echo '<div id="saved_notice" class="admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->args['opt_name']}", __( 'All Defaults Restored!', 'zoomarts' ) ) . '</strong></div>';
                     ...
                     * @param string  translated "settings imported" text
                     */
                    echo '<div id="saved_notice" class="admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->args['opt_name']}", __( 'Section Defaults Restored!', 'zoomarts' ) ) . '</strong></div>';
                } else {
                     ...
                     */
                    echo '<div id="saved_notice" class="admin-notice notice-green"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'zoomarts' ) ) . '</strong></div>';
                }
             ...
             */
            echo '<div id="redux-save-warn" class="notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->args['opt_name']}", __( 'Settings have changed, you should save them!', 'zoomarts' ) ) . '</strong></div>';
            echo '<div id="redux-field-errors" class="notice-red"><strong><span></span> ' . __( 'error(s) were found!', 'zoomarts' ) . '</strong></div>';
            echo '<div id="redux-field-warnings" class="notice-yellow"><strong><span></span> ' . __( 'warning(s) were found!', 'zoomarts' ) . '</strong></div>';
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'zoomarts' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<h3>' . __( 'System Info', 'zoomarts' ) . '</h3>';
